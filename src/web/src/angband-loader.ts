...
class AngbandWorker {
  ...
  gotEvent(evt: shared.KEY_EVENT_MSG) {
    let keyevt: KeyEvent = {
      key: evt.key,
      code: evt.code,
  ...
  public onMessage = (msg: MessageEvent) => {
    let evt = msg.data as shared.WorkerEvent;
    switch (evt.name) {
      case 'KEY_EVENT':
        this.gotEvent(evt as shared.KEY_EVENT_MSG);
        break;
      default:
...
let ANGBAND = new AngbandWorker(self as unknown as Worker);
